...
func run() error {
	...
	if len(os.Args) < 2 {
		return fmt.Errorf("usage: park <add|edit|list|search|show|done|archive|reopen|delete|prune|rename-remote> [flags]")
	}
	...
	case "reopen":
		return cmd.RunReopen(store, args)
	case "delete":
		return cmd.RunDelete(store, args)
	case "prune":
		return cmd.RunPrune(store, args)
	case "rename-remote":
